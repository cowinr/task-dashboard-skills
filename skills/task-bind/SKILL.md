...
- Format: one disciplined line, plain prose, ≤ 140 chars, appended via the `task-note` CLI (which stamps the author and timestamp and refreshes `last-updated`): `task-note T<n> "Blocked on Ben's sign-off before the schema can be finalised."`. `task-note` signs `argus` in an Argus session and `claude` otherwise, so you never write the author or the time by hand. If `task-note` is not on PATH, hand-write `claude YYYY-MM-DD HH:mm — text` (or `argus …` when operating as Argus).
