...
- Format: canonical note format, plain prose, ≤ 140 chars. Lead with `claude` (or `argus` if you are operating as the Argus system — see `task-update`'s author-prefix note), today's date in `YYYY-MM-DD`, and the current wall-clock time in `HH:mm` (24-hour). Example: `claude 2026-05-19 14:32 — Blocked on Ben's sign-off before the schema can be finalised.`
